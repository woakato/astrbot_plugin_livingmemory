...
_RECORD_DEFAULTS: dict[str, dict[str, Any]] = {
    "bot_action": {"memory_type": "self_action", "visibility": "bot_self",
                   "reality_level": "bot_action", "sayability": "direct"},
    "persona_life": {"memory_type": "persona_life", "visibility": "bot_self",
                     "reality_level": "persona_life", "sayability": "indirect"},
    "proactive_message": {"memory_type": "proactive_message", "visibility": "bot_self",
                          "reality_level": "bot_action", "sayability": "direct",
                          "tags": ["proactive", "bot_action"], "importance": 0.55},
    "creative_work": {"memory_type": "creative_work", "visibility": "bot_self",
                      "reality_level": "fictional_content", "sayability": "direct",
                      "tags": ["creative_work"], "importance": 0.72},
    "qzone_action": {"memory_type": "qzone_action", "visibility": "bot_self",
                     "reality_level": "bot_action", "sayability": "direct",
                     "tags": ["qzone", "bot_action"], "importance": 0.58},
    "shared_experience": {"memory_type": "shared_experience", "visibility": "shareable",
                          "reality_level": "bot_action", "sayability": "direct"},
    "search_action": {"memory_type": "search_action", "visibility": "bot_self",
                      "reality_level": "bot_action", "sayability": "indirect"},
    "image_action": {"memory_type": "image_action", "visibility": "bot_self",
                     "reality_level": "bot_action", "sayability": "indirect"},
    "reading": {"memory_type": "reading", "visibility": "bot_self",
                "reality_level": "bot_action", "sayability": "indirect"},
    "schedule_fragment": {"memory_type": "schedule_fragment", "visibility": "bot_self",
                          "reality_level": "bot_action", "sayability": "indirect"},
}

# companion_events kinds for time-window storage (bot_self mirror stream).
_EVENT_KIND_TYPES = frozenset(
    {"persona_life", "proactive_message", "creative_work", "qzone_action",
     "self_action", "schedule_fragment", "search_action", "image_action", "reading"}
)
...
class CompanionBridge:
    ...
    def probe_bot_personal_memory_capabilities(self) -> dict[str, Any]:
        ...
        if not self._active:
            return {
                "available": False, "state": "negative", "capability_state": "negative",
                "degraded": True, "pending": False, "read_only": False,
                "error_code": "bridge_inactive", "p5": {"state": "degraded",
        ...
        probe.update({
            ...
            "pending": False,
            "error_code": "",
            "profiles": ("bot_schedule_current", "bot_schedule_history",
                         "bot_creative", "bot_subjective", "locked_frame_personal"),
    ...
    def _deferred_sections(self) -> set[str]:
        ...
        if self._cfg("prefer_memory_companion_memory", True):
            return {"self_timeline", "private_context", "livingmemory_guidance",
                    "companion_memory", "dialogue_history"}
        return set()

    def coordination_status(self) -> dict[str, Any]:
        if not self._active:
            return {"available": False, "state": "degraded", "degraded": True,
                    "reason": "bridge_inactive"}
        return {
            "available": True, "state": "ready", "degraded": False,
            "schedule_fast_context": bool(self._cfg("schedule_fast_context_enabled", True)),
            ...
            "prefer_memory_companion_memory": bool(self._cfg("prefer_memory_companion_memory", True)),
    ...
    def should_defer_private_companion_section(self, section: str) -> bool:
        return bool(self._cfg("enabled", True)) and str(section) in self._deferred_sections()
    ...
    def get_token_usage_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            usage = {}
        return {
            "token_usage_schema_version": 2, "available": True,
    ...
    async def compose_context(
        ...
        *,
        ...
        """
        if not self._active or not self._cfg("enabled", True):
            return ""
        try:
            return await asyncio.wait_for(
                self._compose_context_inner(
                    query=query, session_context=session_context, top_k=top_k,
                    max_chars=max_chars, companion_bot_mood=companion_bot_mood,
    ...
    async def _compose_context_inner(self, **kwargs: Any) -> str:
        ...
        if profile in {"schedule_fast", "outfit_fast"}:
            ...
            pkg = await self._compose_fast_context(
                composer, bot_id=bot_id, scope=scope, session_id=session_id,
        ...
        pkg = await self._compose_default_context(
            composer, session_id=session_id,
            query=kwargs.get("query") or "",
    ...
    async def _compose_fast_context(self, composer: PackageComposer, *, bot_id: str,
                                    scope: str, session_id: str, query: str,
                                    current_message: str, window_label: str):
        ...
        if store is not None:
            ...
            for row in rows:
                items.append(MemoryItem(text=row["content"], source="companion",
    ...
    async def _compose_default_context(self, composer: PackageComposer, *, session_id: str,
                                       query: str, top_k: int, current_message: str,
                                       window_label: str, mood: str):
        engine = self._engine()
        items: list[MemoryItem] = []
        ...
        if engine is not None and query.strip():
            try:
                results = await engine.search_memories(query=query, k=top_k,
                ...
                for mem in results:
                    ...
                    items.append(MemoryItem(
                        ...
                        time_label="", weight=float(getattr(mem, "final_score", 0.0) or 0.0)))
        ...
        return composer.compose(
            core_blocks=core_blocks, one_line_slots=one_lines, open_loops=loops,
            retrieval_items=items, current_message=current_message,
            window_label=window_label,
        )

    async def _load_core_blocks(self, engine: Any, session_id: str) -> list[dict[str, Any]]:
    ...
    async def record_bot_action(self, *, content: str, **kwargs: Any) -> str:
        return await self._record(content=content, **{**_RECORD_DEFAULTS["bot_action"], **kwargs})

    async def record_persona_life(self, *, content: str, **kwargs: Any) -> str:
        return await self._record(content=content, **{**_RECORD_DEFAULTS["persona_life"], **kwargs})

    async def record_proactive_message(self, *, content: str, **kwargs: Any) -> str:
        return await self._record(content=content,
                                  **{**_RECORD_DEFAULTS["proactive_message"], **kwargs})

    async def record_creative_work(self, *, content: str, **kwargs: Any) -> str:
        return await self._record(content=content,
                                  **{**_RECORD_DEFAULTS["creative_work"], **kwargs})

    async def record_qzone_action(self, *, content: str, **kwargs: Any) -> str:
        return await self._record(content=content,
                                  **{**_RECORD_DEFAULTS["qzone_action"], **kwargs})

    async def record_shared_experience(self, *, content: str, **kwargs: Any) -> str:
        return await self._record(content=content,
                                  **{**_RECORD_DEFAULTS["shared_experience"], **kwargs})

    async def record_search_action(self, *, content: str, **kwargs: Any) -> str:
        return await self._record(content=content,
                                  **{**_RECORD_DEFAULTS["search_action"], **kwargs})

    async def record_image_action(self, *, content: str, **kwargs: Any) -> str:
        return await self._record(content=content,
                                  **{**_RECORD_DEFAULTS["image_action"], **kwargs})

    async def record_reading(self, *, content: str, **kwargs: Any) -> str:
        return await self._record(content=content, **{**_RECORD_DEFAULTS["reading"], **kwargs})

    async def record_schedule_fragment(self, *, content: str, **kwargs: Any) -> str:
        return await self._record(content=content,
                                  **{**_RECORD_DEFAULTS["schedule_fragment"], **kwargs})

    async def record_visible_turn(self, *, role: str = "", content: str = "",
                                  scope: str = "unknown", session_id: str = "",
                                  platform: str = "", user_id: str = "",
                                  user_name: str = "", group_id: str = "",
                                  message_id: str = "", source: str = "external",
        ...
        """
        if not self._active or not content.strip():
            return ""
        store = self._companion_store()
        if store is None:
            return ""
        key = store.make_event_key(
            f"turn:{role}", self._bot_id_of(), scope, session_id, user_id or group_id,
            content, _to_ts(occurred_at))
        ...
        async def _write() -> None:
            ...
            try:
                ...
                sid = session_id or f"{platform}:External:{user_id or group_id or 'unknown'}"
                wanted = content.strip()
                try:
                    recent = await manager.get_messages(sid, limit=20)
                    for msg in reversed(recent):
                        if (getattr(msg, "role", "") == ("assistant" if bot_role else "user")
    ...
    def _bot_id_of(self) -> str:
        try:
            return str(self._plugin.bot_id)
        except Exception:
            return ""

    async def _record(self, *, content: str, memory_type: str = "external_event",
                      scope: str = "unknown", session_id: str = "", platform: str = "",
                      message_id: str = "", group_id: str = "", subject: dict | None = None,
                      object: dict | None = None, visibility: str = "bot_self",
                      sayability: str = "direct", reality_level: str = "bot_action",
                      lifecycle: str = "stable_memory", confidence: float = 0.85,
                      importance: float = 0.5, review_status: str = "auto",
                      tags: list | None = None, metadata: dict | None = None,
                      source_plugin: str = "external", memory_id: str = "",
                      occurred_at: Any = "") -> str:
        """Shared record entry: dedupe, triage, persist off-path (spec §2.3)."""
        if not self._active or not str(content).strip():
            return memory_id or ""
        store = self._companion_store()
        if store is None:
            return memory_id or ""
        ...
        event_key = store.make_event_key(
            str(memory_type), self._bot_id_of(), str(scope), str(session_id),
            user_id or str(group_id), str(content), _to_ts(occurred_at))
        ...
        async def _persist() -> None:
            try:
                _key, deduped = await store.upsert_event(
                    kind=str(memory_type), content=str(content)[:4000],
                    bot_id=self._bot_id_of(), persona_id=str(meta.get("persona_id") or ""),
                    scope=str(scope), session_id=str(session_id),
                    user_id=user_id, group_id=str(group_id),
                    window_slug=str(meta.get("source_window") or ""),
                    occurred_at=_to_ts(occurred_at), importance=float(importance),
                    metadata=meta, event_key=event_key)
                if deduped:
    ...
    async def record_bot_personal_archive(self, envelope: Any, *, producer_capability: Any = None,
                                          producer_context: Any = None) -> dict[str, Any]:
        """Store one schedule/diary archive envelope (spec §2.3 fixed 6 keys)."""
        if not self._active:
            return {"ok": False, "record_id": "", "deduplicated": False, "version": 0,
                    "error_code": "bridge_inactive", "state": "degraded"}
        if not isinstance(producer_capability, _ProducerCapability):
            return {"ok": False, "record_id": "", "deduplicated": False, "version": 0,
                    "error_code": "producer_capability_required", "state": "forbidden"}
        if not isinstance(envelope, dict):
            envelope = getattr(envelope, "__dict__", None)
            if not isinstance(envelope, dict):
                return {"ok": False, "record_id": "", "deduplicated": False, "version": 0,
                        "error_code": "envelope_invalid", "state": "invalid"}
        ...
        if memory_type not in _ARCHIVE_TYPES:
            return {"ok": False, "record_id": record_id, "deduplicated": False,
                    "version": 0, "error_code": "memory_type_rejected", "state": "invalid"}
        ...
        if bot_hook and owner_bot_id and bot_hook != owner_bot_id:
            return {"ok": False, "record_id": record_id, "deduplicated": False,
                    "version": 0, "error_code": "producer_namespace_mismatch", "state": "forbidden"}
        ...
        try:
            ...
            payload_text = "{}"
        if len(payload_text.encode("utf-8")) > bot_personal_contract.BOT_PERSONAL_MAX_PAYLOAD_BYTES:
            return {"ok": False, "record_id": record_id, "deduplicated": False,
                    "version": 0, "error_code": "payload_too_large", "state": "invalid"}
        version = int(envelope.get("version") or 1)
        idem = _s("idempotency_key", 160)
        if not idem:
            return {"ok": False, "record_id": record_id, "deduplicated": False,
                    "version": 0, "error_code": "idempotency_key_required", "state": "invalid"}
        store = self._companion_store()
        if store is None:
            return {"ok": False, "record_id": record_id, "deduplicated": False,
                    "version": 0, "error_code": "store_unavailable", "state": "degraded"}
        ...
        try:
            _key, deduped = await store.upsert_event(
                kind="archive", content=summary,
                bot_id=owner_bot_id or self._bot_id_of(), persona_id=persona_id,
                scope="private", session_id="bot_personal", user_id="", group_id="",
                window_slug=window, occurred_at=occurred, importance=0.55,
                metadata={"archive_version": version,
                ...
                event_key=f"lmce_arch_{abs(hash(idem)) & ((1 << 96) - 1):024x}")
        except Exception as exc:  # noqa: BLE001
            logger.warning(f"[companion_bridge] archive failed: {exc}")
            return {"ok": False, "record_id": record_id, "deduplicated": False,
                    "version": 0, "error_code": "store_error", "state": "degraded"}
        return {"ok": True, "record_id": record_id, "deduplicated": bool(deduped),
                "version": version, "error_code": None,
                "state": "deduplicated" if deduped else "sent"}
    ...
    def register_emotion_producer(self, producer: Any) -> Any | None:
        if not self._active or producer is None:
            return None
        meta = self._match_pc_star(producer)
        if meta is None:
            logger.warning("[companion_bridge] emotion producer registration refused: "
                           "producer is not a live private_companion star")
            return None
        bot_id, persona_id = self._producer_namespace(producer)
        capability = _ProducerCapability(token=f"{self._token_epoch}:{uuid.uuid4().hex[:16]}",
                                         bot_id=bot_id, persona_id=persona_id)
    ...
    def _producer_namespace(producer: Any) -> tuple[str, str]:
        ...
        bot_id = str(bot_hook()) if callable(bot_hook) else ""
    ...
    def create_emotion_producer_context(self, capability: Any, *, bot_id: str, scope: str,
                                        platform: str, user_id: str,
                                        session_id: str) -> Any | None:
        return self._make_context(capability, bot_id=bot_id, scope=scope, platform=platform,
                                  user_id=user_id, session_id=session_id)

    def create_user_memory_context(self, capability: Any, **kwargs: Any) -> Any | None:
        return self._make_context(capability, **kwargs)

    def create_emotion_delivery_context(self, capability: Any, *, bot_id: str, scope: str,
                                        platform: str, user_id: str, session_id: str,
                                        consumer_id: str = _EMOTION_CONSUMER_ID,
                                        allow_cross_window: bool = False) -> Any | None:
        ...
        if allow_cross_window and not self._cfg(
            ...
            return None
        base = self._make_context(capability, bot_id=bot_id, scope=scope, platform=platform,
                                  user_id=user_id, session_id=session_id)
        if base is None:
            return None
        return _DeliveryContext(token=base.token, bot_id=base.bot_id, scope=base.scope,
                                platform=base.platform, user_id=base.user_id,
                                session_id=base.session_id, consumer_id=consumer_id,
                                allow_cross_window=allow_cross_window)

    def _make_context(self, capability: Any, *, bot_id: str = "", scope: str = "",
                      platform: str = "", user_id: str = "", session_id: str = "",
                      **_ignored: Any) -> _ProducerContext | None:
        ...
        if not str(session_id).startswith(f"{platform}:"):
            return None
        return _ProducerContext(token=capability.token, bot_id=str(bot_id), scope="private",
                                platform=str(platform), user_id=str(user_id),
                                session_id=str(session_id))

    async def record_emotion_event(self, event: Any, *, producer_context: Any = None) -> dict[str, Any]:
        forbidden = {"ok": False, "state": "forbidden", "read_only": False,
                     "event_id": "", "error_code": "producer_context_required"}
        if not isinstance(producer_context, _ProducerContext):
            return forbidden
        store = self._companion_store()
        if store is None:
            return {"ok": False, "state": "degraded", "read_only": False,
                    "event_id": "", "error_code": "store_unavailable"}
        attested = dict(event) if isinstance(event, dict) else {}
        attested.update({
            "producer_plugin": "private_companion",
            "bot_id": producer_context.bot_id, "scope": "private",
            "platform": producer_context.platform,
            "session_id": producer_context.session_id,
            "target_ref": attested.get("target_ref") or {"id": producer_context.user_id},
        })
        ...
        context_fields = {"bot_id": producer_context.bot_id, "scope": "private",
                          "platform": producer_context.platform,
        ...
        try:
            ...
            return {"ok": False, "state": "degraded", "read_only": False,
                    "event_id": normalized.get("event_id", ""), "error_code": "store_error"}
        stored["ok"] = True
        return stored

    async def list_emotion_events(self, *, delivery_context: Any = None, cursor: str = "",
                                  limit: int = 10, **_legacy: Any) -> dict[str, Any]:
        forbidden = {"schema_version": "emotion_afterglow_delivery.v1", "state": "forbidden",
                     "read_only": True, "events": [], "next_cursor": "", "has_more": False,
                     "error_code": "delivery_context_required"}
        if not isinstance(delivery_context, _DeliveryContext):
            return forbidden
        store = self._companion_store()
        if store is None:
            return {"schema_version": "emotion_afterglow_delivery.v1", "state": "degraded",
                    "read_only": True, "events": [], "next_cursor": "", "has_more": False,
                    "error_code": "store_unavailable"}
        try:
            events = await store.list_deliverable(
                scope=delivery_context.scope, platform=delivery_context.platform,
                user_id=delivery_context.user_id, session_id=delivery_context.session_id,
        ...
        return {"schema_version": "emotion_afterglow_delivery.v1", "state": "ready",
                "read_only": True, "events": events, "next_cursor": "",
                "has_more": False}

    async def ack_emotion_events(self, event_refs: Any, *, delivery_context: Any = None,
                                 **_legacy: Any) -> dict[str, Any]:
        forbidden = {"state": "forbidden", "acked": 0,
                     "error_code": "delivery_context_required"}
        if not isinstance(delivery_context, _DeliveryContext):
            return forbidden
        ...
        if store is None:
            return forbidden
        try:
            acked = await store.ack([r for r in event_refs[:64] if isinstance(r, dict)],
                                    consumer_id=delivery_context.consumer_id)
        except Exception as exc:  # noqa: BLE001
            logger.warning(f"[companion_bridge] ack failed: {exc}")
            return forbidden
        return {"acked": int(acked), "consumer_id": delivery_context.consumer_id,
    ...
    def get_relationship_phase(self, *, session_id: str = "", scope: str = "private",
                               platform: str = "", user_id: str = "",
                               group_id: str = "", bot_id: str = "") -> dict[str, Any]:
        # v1: honest unknown shape; real estimation arrives with the portrait.
        return {"phase": "unknown", "momentum": 0.0}

    def peek_relationship_phase(self, *, session_id: str = "", scope: str = "private",
                                platform: str = "", user_id: str = "",
                                group_id: str = "", bot_id: str = "") -> dict[str, Any]:
...
def _extract_payload_summary(payload: dict) -> str:
    ...
    for key in ("summary", "text", "content", "title", "description", "plan", "activity"):
