...
async def collect_slots(
    *,
    ...
    if get("companion_slots.enable_core_block", True) and get(
        "core_memory.enabled", True
    ):
        try:
        ...
        try:
            rows = await companion_store.peek_pending(
                ...
                allow_cross_window=bool(
                    get(
                        "companion_bridge.cross_window_emotional_continuity_enabled",
                        False,
                    )
                ),
                limit=3,
            ...
            for row in rows:
                ...
                if phrase:
                    bundle.one_line_slots.append(
                        ("emotional_hint", f"情绪余波：{phrase}")
                    )
                    break
        except Exception as exc:  # noqa: BLE001
    ...
    if get("companion_slots.enable_open_loops", True):
        try:
            ...
            bundle.open_loops = [
                MemoryItem(
                    text=_loop_text(item),
                    source="open_loop",
                    time_label=f"约{item['age_days']}天前"
                    if item.get("age_days")
                    else "",
                    weight=(
                        2.0
                        if item.get("due_ts") and item["due_ts"] < time.time()
                        else 0.0
                    )
    ...
    suppress_self = bool(
        get("companion_bridge.suppress_self_timeline_when_companion_seen", True)
    )
    if (
        companion_store is not None
        and get("companion_slots.enable_self_line", True)
        ...
        try:
            ...
            rows = await companion_store.list_events_between(
                kinds=_SELF_KINDS, since_date=today, until_date=today, limit=6
            )
            texts = [
                str(r.get("content") or "").strip() for r in rows if r.get("content")
            ]
            if texts:
                bundle.one_line_slots.append(
                    ("self_timeline", "今天：" + "；".join(texts[:2])[:36])
                )
        except Exception as exc:  # noqa: BLE001
...
async def _relationship_slots(
    *, companion_store: Any, user_id: str, platform: str, session_message_count: int = 0
) -> list[tuple[str, str]]:
